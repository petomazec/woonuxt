...
if ( ! function_exists( 'ogami_body_classes' ) ) {
	function ogami_body_classes( $classes ) {
		global $post;
		$show_footer_mobile = ogami_get_config('show_footer_mobile', true);
		...
		if ( ogami_get_config('preload', true) ) {
			$classes[] = 'apus-body-loading';
		}
		if ( ogami_get_config('image_lazy_loading') ) {
	...
	add_filter( 'body_class', 'ogami_body_classes' );
}

if ( ! function_exists( 'ogami_get_shortcode_regex' ) ) {
	function ogami_get_shortcode_regex( $tagregexp = '' ) {
...
if ( ! function_exists( 'ogami_tagregexp' ) ) {
	function ogami_tagregexp() {
		return apply_filters( 'ogami_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|ogami_media' );
	}
}

if ( !function_exists('ogami_get_header_layouts') ) {
	function ogami_get_header_layouts() {
		$headers = array();
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
...
if ( !function_exists('ogami_get_header_layout') ) {
	function ogami_get_header_layout() {
		global $post;
		if ( is_page() && is_object($post) && isset($post->ID) ) {
			...
			if ( empty($header) || $header == 'global' ) {
				return ogami_get_config('header_type');
			}
			return $header;
		}
		return ogami_get_config('header_type');
	}
	add_filter( 'ogami_get_header_layout', 'ogami_get_header_layout' );
}

function ogami_display_header_builder($header_slug) {
	$args = array(
		'name'        => $header_slug,
		'post_type'   => 'apus_header',
		'post_status' => 'publish',
	...
	foreach ( $posts as $post ) {
		...
		if ( ogami_get_config('keep_header') ) {
			echo '<div class="main-sticky-header-wrapper">';
	        echo '<div class="main-sticky-header">';
	    }
			echo apply_filters( 'ogami_generate_post_builder', do_shortcode( $post->post_content ), $post, $post->ID);
		if ( ogami_get_config('keep_header') ) {
			echo '</div>';
	        echo '</div>';
	    }
...
if ( !function_exists('ogami_get_footer_layouts') ) {
	function ogami_get_footer_layouts() {
		$footers = array();
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
...
if ( !function_exists('ogami_get_footer_layout') ) {
	function ogami_get_footer_layout() {
		if ( is_page() ) {
			global $post;
			$footer = '';
			if ( is_object($post) && isset($post->ID) ) {
				...
				if ( empty($footer) || $footer == 'global' ) {
					return ogami_get_config('footer_type', '');
				}
			}
			return $footer;
		}
	...
	add_filter('ogami_get_footer_layout', 'ogami_get_footer_layout');
}

function ogami_display_footer_builder($footer_slug) {
	$show_footer_desktop_mobile = ogami_get_config('show_footer_desktop_mobile', false);
	$args = array(
		'name'        => $footer_slug,
		'post_type'   => 'apus_footer',
		'post_status' => 'publish',
...
if ( !function_exists('ogami_blog_content_class') ) {
	function ogami_blog_content_class( $class ) {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		if ( ogami_get_config('blog_'.$page.'_fullwidth') ) {
			return 'container-fluid';
		}
		return $class;
	}
}
add_filter( 'ogami_blog_content_class', 'ogami_blog_content_class', 1 , 1  );


if ( !function_exists('ogami_get_blog_layout_configs') ) {
	function ogami_get_blog_layout_configs() {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		$left = ogami_get_config('blog_'.$page.'_left_sidebar');
		$right = ogami_get_config('blog_'.$page.'_right_sidebar');

		switch ( ogami_get_config('blog_'.$page.'_layout') ) {
		 	case 'left-main':
...
if ( !function_exists('ogami_page_content_class') ) {
	function ogami_page_content_class( $class ) {
		global $post;
		if (is_object($post)) {
...
add_filter( 'ogami_page_content_class', 'ogami_page_content_class', 1 , 1  );

if ( !function_exists('ogami_get_page_layout_configs') ) {
	function ogami_get_page_layout_configs() {
		global $post;
		if ( is_object($post) ) {
...
if ( ! function_exists( 'ogami_get_first_url_from_string' ) ) {
	function ogami_get_first_url_from_string( $string ) {
...
if ( !function_exists( 'ogami_get_link_attributes' ) ) {
	function ogami_get_link_attributes( $string ) {
...
if ( !function_exists( 'ogami_post_media' ) ) {
	function ogami_post_media( $content ) {
		$is_video = ( get_post_format() == 'video' ) ? true : false;
		$media = ogami_get_first_url_from_string( $content );
		$media = $media['link'];
		if ( ! empty( $media ) ) {
			global $wp_embed;
			$content = do_shortcode( $wp_embed->run_shortcode( '[embed]' . $media . '[/embed]' ) );
		} else {
			$pattern = ogami_get_shortcode_regex( ogami_tagregexp() );
...
if ( !function_exists( 'ogami_post_gallery' ) ) {
	function ogami_post_gallery( $content, $args = array() ) {
		$output = '';
		$defaults = array( 'size' => 'large' );
		$args = wp_parse_args( $args, $defaults );
	    $gallery_filter = ogami_gallery_from_content( $content );
...
if (!function_exists('ogami_gallery_from_content')) {
    function ogami_gallery_from_content($content) {
...
if ( !function_exists( 'ogami_random_key' ) ) {
    function ogami_random_key($length = 5) {
...
if ( !function_exists('ogami_substring') ) {
    function ogami_substring($string, $limit, $afterlimit = '[...]') {
        if ( empty($string) ) {
        	return $string;
        }
...
function ogami_is_apus_framework_activated() {
	return defined('APUS_FRAMEWORK_VERSION') ? true : false;
}

function ogami_is_cmb2_activated() {
	return defined('CMB2_LOADED') ? true : false;
}

function ogami_is_woocommerce_activated() {
	return class_exists( 'woocommerce' ) ? true : false;
}

function ogami_is_yith_wcwl_activated() {
	return class_exists( 'YITH_WCWL' ) ? true : false;
}

function ogami_is_yith_woocompare_activated() {
	return class_exists( 'YITH_Woocompare' ) ? true : false;
}

function ogami_is_revslider_activated() {
	return class_exists( 'RevSlider' ) ? true : false;
}

function ogami_is_dokan_activated() {
	return class_exists( 'WeDevs_Dokan' ) ? true : false;
}

function ogami_is_woo_swatches_activated() {
	return class_exists( 'TA_WC_Variation_Swatches' ) ? true : false;
}

function ogami_is_mailchimp_activated() {
